...
function AdminSettings() {
  ...
  const [gatewayForm, setGatewayForm] = useState<GatewayCreds>({
    provider: "none",
    api_key: "",
    secret_key: "",
    webhook_secret: "",
  ...
  useEffect(() => {
    if (settings) {
      const vert =
        (((settings as any)?.business_vertical ||
          detectVerticalFromStoreName(settings?.store_name)) as BusinessVertical) || "seafood";
      setLiveChatConfig(
        getSavedLiveChatConfig(vert, settings.store_name, settings.contact_phone, tenant.tenantId),
      );
    }
  ...
  const handleResetLiveChatToVertical = () => {
    const vert = (form.business_vertical ||
      (settings as any)?.business_vertical ||
      detectVerticalFromStoreName(form.store_name || settings?.store_name) ||
      "seafood") as BusinessVertical;
    const defaults = getDefaultLiveChatConfig(
      vert,
  ...
  const handleSeedConfirm = async (options: {
    archiveExisting: boolean;
    keepCustomProducts: boolean;
  }) => {
    try {
      ...
      if (res.success) {
        toast.success(
          `Successfully loaded ${res.inserted} authentic ${seedVerticalTarget.shortName} products!`,
        );
  ...
  const handleResetLayoutConfig = () => {
    const defaultCfg = getDefaultProductLayoutConfig(
      form.business_vertical || settings?.business_vertical,
      form.store_name || settings?.store_name,
    );
  ...
  const handleAddCustomMethod = () => {
    ...
    const id = newMethodName
      ...
      .replace(/[^a-z0-9]/g, "_");
    if (
      ["cash", "upi", "card", "split"].includes(id) ||
      paymentConfig.customMethods.some((m) => m.id === id)
    ) {
  ...
  const handleExportOrdersCsv = async () => {
    try {
      ...
      const { data: orders, error } = await supabase
        .from("orders")
        .select(
          "id, created_at, status, payment_status, payment_method, total, subtotal, delivery_fee, discount, customer_name, customer_phone, delivery_address",
        )
        .order("created_at", { ascending: false })
        .limit(3000);
      ...
      link.setAttribute(
        "download",
        `orders-ledger-${tenant.tenantId}-${new Date().toISOString().split("T")[0]}.csv`,
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const handleExportProductsCsv = async () => {
    try {
      ...
      link.setAttribute(
        "download",
        `products-catalog-${tenant.tenantId}-${new Date().toISOString().split("T")[0]}.csv`,
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const update = useMutation({
    mutationFn: async (patch: any) => {
      ...
      if (shipping_scope_config) {
        dbPatch.terms_content =
          typeof shipping_scope_config === "string"
  ...
  const handleSaveShippingScope = async () => {
    ...
    try {
      ...
      toast.success(
        `Coverage Saved: ${shippingScope.scope_mode.toUpperCase()} mode active across all devices!`,
      );
    } catch (e: any) {
  ...
  const handleToggleLiveAlerts = async (checked: boolean) => {
    ...
    try {
      ...
      toast.success(
        checked
          ? "Live Catch Alert Banner enabled!"
          : "Live Catch Alert Banner disabled on storefront!",
      );
    } catch (e: any) {
  ...
  const [activeTab, setActiveTab] = useState<
    "general" | "branches" | "delivery" | "payments" | "growth" | "system"
  >("general");
  ...
  const SETTINGS_SECTIONS: SettingSectionItem[] = useMemo(
    () => [
              ...
              <div className="flex items-center justify-between border-b border-border/50 pb-4">
                <div className="space-y-0.5 max-w-sm">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    If disabled, customers cannot click into a product to view the full
                    gallery/details. They can only add it directly to cart from the catalog list.
                  </p>
                </div>
                <Switch
      ...
      },
      {
        ...
        description:
          "Manage store logo, brand name, and public tagline. Auto-compressed to under 500KB.",
        keywords: [
          "logo",
          "brand",
          "identity",
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Manage your store logo and public identity. Upload, edit, or delete/reset your
                    logo. Auto-compressed to under 500KB.
                  </p>
                </div>
                <Badge
                  variant="outline"
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 rounded-2xl border border-border/80 bg-muted/20 p-4">
                ...
                <div className="space-y-1.5 flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Target size: ≤ 500 KB. High-res vector and raster images are automatically
                    compressed to ensure lightning fast load on mobile networks & PWA home screens.
                  </p>
                  ...
                  <div className="pt-2 flex flex-wrap items-center gap-2">
                    <ImageUpload
                      ...
                      onUpload={(url) => {
                        ...
                        toast.success(
                          "New brand logo uploaded & optimized! Click 'Save Settings' below to persist.",
                        );
                      }}
                      onRemove={
                        form.logo_url
                          ? () => {
                              ...
                              toast.info(
                                "Custom logo removed. Master logo restored! Click 'Save Settings' to persist.",
                              );
                            }
                          : undefined
                      }
                    ...
                    {form.logo_url && (
                      <Button
                        ...
                        onClick={() => {
                          setForm({ ...form, logo_url: null, shop_logo: null });
                          toast.info(
                            "Custom logo removed. Default master logo restored! Click 'Save Settings' to persist.",
                          );
                        }}
                      >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                    ...
                    {form.store_name && (
                      <span className="text-[10px] text-muted-foreground">
                        ...
                        <strong className="text-primary font-semibold">
                          {
                            VERTICAL_CONFIGS[detectVerticalFromStoreName(form.store_name)]
                              ?.shortName
                          }
                        </strong>
                      </span>
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder={
                      VERTICAL_CONFIGS[
                        (form.business_vertical as BusinessVertical) ||
                          detectVerticalFromStoreName(form.store_name) ||
                          "seafood"
                      ]?.tagline || "Fresh Products Delivered Daily"
                    }
                    className="mt-1 text-sm rounded-xl"
                  />
                </div>
              ...
              <div className="pt-1">
                ...
                <p className="text-[10px] text-muted-foreground mb-2">
                  Select the initial language for new visitors. Users can still change their
                  preferred language via the header.
                </p>
                <select
              ...
              {(() => {
                ...
                const currentVertical = (form.business_vertical ||
                  settings?.business_vertical ||
                  "seafood") as BusinessVertical;
                  ...
                  <div className="mt-3 p-3.5 rounded-2xl border border-primary/30 bg-primary/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-in fade-in duration-300">
                    <div className="space-y-0.5 min-w-0">
                      <div className="flex items-center gap-1.5 text-xs font-bold text-foreground">
                        ...
                        <span>
                          Store name suggests <strong>{inferredCfg.name}</strong>
                        </span>
                        <Badge
                          variant="outline"
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Your current vertical is set to{" "}
                        {VERTICAL_CONFIGS[currentVertical]?.name || currentVertical}. Switch
                        verticals and synchronize catalog presets?
                      </p>
                    </div>
      ...
      },
      {
        ...
        description:
          "Switch store model between Coastal Seafood, Poultry & Meat, Multi-Meat Superstore, or Universal Mart.",
        keywords: [
          "vertical",
          "industry",
          "seafood",
            ...
            <CardHeader className="pb-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Instantly switch this app between Seafood, Chicken & Meat, or a Multi-Meat
                    Superstore without losing any operational systems.
                  </p>
                </div>
                <Badge
                  variant="outline"
                  ...
                  <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                    <div className="text-xs">
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        Sets theme color ({currentCfg.recommendedThemeColor}), tagline, and
                        guarantee badge.
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2 shrink-0">
                      ...
                      >
                        ...
                        {seedingCatalog ? "Loading…" : `Seed ${currentCfg.shortName} Products`}
                      </Button>
                    </div>
                  </div>
                );
              ...
              <div>
                ...
                <div className="mt-1.5">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-1">
                    Upload a showcase banner for this vertical (e.g. fresh farm chicken cuts or
                    seafood harvest display).
                  </p>
                </div>
              </div>
            </CardContent>
      ...
      },
      {
        ...
        tab: "general" as const,
        tabLabel: "General & Brand",
        title: "Store Ordering & Sales Channel Mode",
        description:
          "Choose how customers interact with your store: Full Online Checkout, Hybrid WhatsApp, WhatsApp Deep-Link Only, or Catalog Showcase Only.",
        keywords: [
          "ordering mode",
          "whatsapp order",
          "catalog only",
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <CardTitle className="text-lg flex items-center gap-2">
                    ...
                    Store Ordering &amp; Sales Channel Mode
                  </CardTitle>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Configure checkout flow for your store. Standard online checkout is active by
                    default for all existing and upcoming stores.
                  </p>
                </div>
                <Badge
                  variant="outline"
            ...
            <CardContent className="space-y-4">
              ...
              >
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <Label
                        htmlFor="whatsapp-exclusive-toggle"
                    ...
                    <p className="text-xs text-muted-foreground max-w-xl leading-relaxed">
                      Enable this to hide standard online checkout and payment gateways. Customers
                      will browse your catalog, build carts, and place orders exclusively via
                      WhatsApp with itemized catch and pricing details.
                    </p>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(checked) => {
                      ...
                      if (checked) {
                        toast.info(
                          "WhatsApp Only Ordering activated: Online checkout options are hidden.",
                        );
                      } else {
                ...
                {form.ordering_mode === "whatsapp_only" && (
                  <div className="mt-3 pt-3 border-t border-emerald-500/20 flex items-center gap-2 text-xs text-emerald-800 dark:text-emerald-300">
                    ...
                    <span>
                      Other checkout methods are hidden across the store. Cart will display direct
                      WhatsApp order only.
                    </span>
                  </div>
                )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  {
                    id: "standard",
                    title: "Standard Online Checkout",
                    subtitle: "Default for all stores",
                    desc: "Full e-commerce checkout with payment gateways, Cash on Delivery, order tracking, and POS sync.",
                    badge: "Default Flow",
                    icon: CreditCard,
                  },
                  {
                    id: "both",
                    title: "Hybrid (Online + WhatsApp)",
                    subtitle: "Best Conversion",
                    desc: "Customers can either pay online via standard checkout OR tap 1-click WhatsApp order directly from cart.",
                    badge: "Highest Flexibility",
                    icon: Zap,
                  },
                  {
                    id: "whatsapp_only",
                    title: "WhatsApp Order Only",
                    subtitle: "No Gateway Required",
                    desc: "Customers build cart, select cutting style, and send rich itemized order details to your WhatsApp number via deep link.",
                    badge: "Direct Chat",
                    icon: MessageSquare,
                  },
                  {
                    id: "catalog_only",
                    title: "Catalog Showcase Only",
                    subtitle: "Digital Menu / Catalog",
                    desc: "Customers can browse daily fresh arrivals, prices, and species details; orders are placed via store inquiry.",
                    badge: "Showcase Only",
                    icon: Store,
                  },
                ].map((mode) => {
                  const isSelected = (form.ordering_mode || "standard") === mode.id;
                  ...
                  return (
                        ...
                        if (typeof window !== "undefined") {
                          ...
                          window.dispatchEvent(
                            new CustomEvent("fishnfresh:ordering-mode-updated", {
                              detail: {
                                ordering_mode: newMode,
                                whatsapp_order_phone: form.whatsapp_order_phone,
                              },
                            }),
                          );
                        }
                        toast.success(
                          `Selected "${mode.title}". Click "Save Channel Settings" below to persist permanently.`,
                        );
                      }}
                    ...
                    >
                      ...
                      <div className="pt-3 mt-2 border-t border-border/50 flex items-center justify-between text-xs">
                        ...
                        >
                          {isSelected ? "● Selected Mode" : "Select"}
                        </span>
              ...
              <div className="p-4 rounded-2xl border bg-muted/20 space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Orders dispatched via WhatsApp deep link will be sent directly to this
                      WhatsApp chat.
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-3 border-t border-border/60 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          toast.success(
                            `Store Sales Channel saved! Mode: ${mode === "both" ? "Hybrid (Online + WhatsApp)" : mode}`,
                          );
                        } catch (e: any) {
                    ...
                    >
                      ...
                      <span>
                        {update.isPending ? "Saving Changes..." : "Save Channel Settings"}
                      </span>
                    </Button>
                  ...
                  {(form.whatsapp_order_phone || form.contact_phone) && (
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const p = (form.whatsapp_order_phone || form.contact_phone || "").replace(
                          /\D/g,
                          "",
                        );
                        const num = p.length === 10 ? `91${p}` : p;
                        window.open(
                          `https://wa.me/${num}?text=Hello!%20Testing%20store%20WhatsApp%20ordering%20desk.`,
                          "_blank",
                        );
                      }}
              ...
              <div className="p-4 rounded-2xl border border-emerald-500/30 bg-emerald-50/50 dark:bg-emerald-950/20 space-y-3 text-xs">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2 text-emerald-950 dark:text-emerald-200">
                    ...
                    <span className="font-bold text-sm">
                      WhatsApp Business Auto-Reply &amp; Quick Replies Setup
                    </span>
                  </div>
                  <Badge
                    variant="outline"
                    className="text-[10px] border-emerald-500/40 text-emerald-700 dark:text-emerald-300 bg-white/80 dark:bg-black/40"
                  >
                    Official WhatsApp Business
                  </Badge>
                </div>
                <p className="text-muted-foreground leading-relaxed">
                  When customers place an order, our system attaches their itemized catch
                  specifications, custom cut styles, live doorstep Google Maps navigation link, and
                  tracking URL. Set up an automated receipt confirmation in your WhatsApp Business
                  app so customers get instant acknowledgement.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                  <div className="p-3 rounded-xl bg-background border border-border space-y-1.5">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      In WhatsApp Business &rarr;{" "}
                      <strong>Settings &rarr; Business Tools &rarr; Quick Replies</strong>. Tap{" "}
                      <strong>+</strong>, set Shortcut to <code>/confirm</code> and paste the
                      template.
                    </p>
                  </div>
                  <div className="p-3 rounded-xl bg-background border border-border space-y-1.5">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      In WhatsApp Business &rarr; <strong>Away Message</strong> or{" "}
                      <strong>Greeting Message</strong>. Enable "Send greeting message" and paste
                      the template for instant 24/7 replies.
                    </p>
                  </div>
                </div>
...
If you need any cut modifications, please reply here. Thank you!`}
      ...
      },
      {
        ...
        description:
          "Deeply customize live support desk title, welcome greeting, support channels, and automated responses for freshness, cuts, GPS tracking, and delivery SLAs isolated per store vertical.",
        keywords: [
          "live support",
          "chat widget",
          "auto reply",
            ...
            <CardHeader className="pb-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Multi-vertical isolated live support engine. Ensure your desk name, welcome
                    greeting, 1-tap FAQ chips, and instant auto-replies accurately reflect your
                    store's vertical.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setLiveChatConfig({ ...liveChatConfig, botName: e.target.value })
                    }
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setLiveChatConfig({ ...liveChatConfig, welcomeMessage: e.target.value })
                    }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setLiveChatConfig({ ...liveChatConfig, supportPhone: e.target.value })
                    }
                    placeholder="+91 98430 61919"
                    className="mt-1 text-sm rounded-xl"
                  />
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setLiveChatConfig({ ...liveChatConfig, whatsappNumber: e.target.value })
                    }
                    placeholder="+91 98430 61919"
                    className="mt-1 text-sm rounded-xl"
                  />
              ...
              <div className="space-y-4 pt-2 border-t border-border/60">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Tailor replies to your specific retail products. Avoid seafood-specific text
                      if you sell poultry, groceries, or goods.
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <Label className="text-xs font-semibold">
                      1. Freshness &amp; Sourcing Auto-Reply
                    </Label>
                    <Textarea
                      rows={3}
                      value={liveChatConfig.freshnessReply}
                      onChange={(e) =>
                        setLiveChatConfig({ ...liveChatConfig, freshnessReply: e.target.value })
                      }
                  ...
                  <div>
                    <Label className="text-xs font-semibold">
                      2. Custom Cuts &amp; Preparation Auto-Reply
                    </Label>
                    <Textarea
                      rows={3}
                      value={liveChatConfig.cutsReply}
                      onChange={(e) =>
                        setLiveChatConfig({ ...liveChatConfig, cutsReply: e.target.value })
                      }
                  ...
                  <div>
                    <Label className="text-xs font-semibold">
                      3. Live GPS Radar &amp; PIN Auto-Reply
                    </Label>
                    <Textarea
                      rows={3}
                      value={liveChatConfig.trackingReply}
                      onChange={(e) =>
                        setLiveChatConfig({ ...liveChatConfig, trackingReply: e.target.value })
                      }
                  ...
                  <div>
                    <Label className="text-xs font-semibold">
                      4. Delivery Speed &amp; Time Slot Auto-Reply
                    </Label>
                    <Textarea
                      rows={3}
                      value={liveChatConfig.deliveryTimeReply}
                      onChange={(e) =>
                        setLiveChatConfig({ ...liveChatConfig, deliveryTimeReply: e.target.value })
                      }
                  ...
                  <div>
                    <Label className="text-xs font-semibold">
                      5. 100% Guarantee &amp; Replacement SLA
                    </Label>
                    <Textarea
                      rows={3}
                      value={liveChatConfig.guaranteeReply}
                      onChange={(e) =>
                        setLiveChatConfig({ ...liveChatConfig, guaranteeReply: e.target.value })
                      }
                  ...
                  <div>
                    <Label className="text-xs font-semibold">
                      6. Supported Payment Methods Auto-Reply
                    </Label>
                    <Textarea
                      rows={3}
                      value={liveChatConfig.paymentReply}
                      onChange={(e) =>
                        setLiveChatConfig({ ...liveChatConfig, paymentReply: e.target.value })
                      }
              ...
              <div className="space-y-3 pt-2 border-t border-border/60">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      These buttons appear directly in the chat widget. Customers tap once to
                      instantly receive the exact verified answer without typing.
                    </p>
                  </div>
                  <Button
                    type="button"
                    ...
                    onClick={() => {
                      if (!newFaqQ.trim() || !newFaqA.trim()) {
                        toast.error(
                          "Please provide both a Question and an Answer for the FAQ chip.",
                        );
                        return;
                      }
                      setLiveChatConfig({
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                  {liveChatConfig.customFaqs.map((faq, idx) => (
                    <div
                      key={idx}
                      className="p-3 rounded-2xl border border-border/80 bg-muted/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3"
                    >
                      <div className="space-y-1 flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground line-clamp-2">{faq.a}</p>
                      </div>
                      <Button
                        type="button"
                        variant="ghost"
      ...
      },
      {
        ...
        description:
          "Granularly control which sections (Nutrition, Culinary Specs, Portion Chips, Subscriptions, AI Intelligence, Reviews) appear on product pages and customize their titles.",
        keywords: [
          "product layout",
          "customization",
          "nutrition",
            ...
            <CardHeader className="pb-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Tailor exactly which sections, specifications, and text labels appear across all
                    product pages for your store vertical.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const updated = {
                              ...layoutConfig,
                              subscriptionSubtitle: e.target.value,
                            };
                            setLayoutConfig(updated);
                            saveProductLayoutConfig(updated, tenant.tenantId);
                          }}
                ...
                <div className="rounded-2xl border border-border/70 p-3.5 bg-card/60 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5 pr-2">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Multi-language health highlights, taste profile, and culinary preparation
                        tips.
                      </p>
                    </div>
                    <Switch
                ...
                <div className="rounded-2xl border border-border/70 p-3.5 bg-card/60 space-y-2.5 md:col-span-2">
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5 pr-2">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Automated cross-sell carousel featuring matching items from the same
                        category.
                      </p>
                    </div>
                    <Switch
      ...
      },
      {
        ...
        description:
          "Theme color palettes, daily coastal atmosphere engine, footer address, Google Maps link, and social profiles.",
        keywords: [
          "theme",
          "color",
          "palette",
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3 rounded-xl border border-primary/20 bg-primary/5 p-3.5">
                  ...
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <Label className="text-sm font-bold text-foreground">
                        Infinite Daily Coastal Atmosphere
                      </Label>
                      <Badge
                        variant="outline"
                        className="text-[10px] py-0 border-primary/40 text-primary bg-primary/10"
                      >
                        Dynamic Mood Engine
                      </Badge>
                    </div>
                    <p className="text-[11px] text-muted-foreground leading-snug">
                      Procedurally shifts the coastal atmosphere, maritime palette, and status bar
                      color every single day (14 rotating coastal moods with infinite
                      micro-variations). Gives customers a fresh, premium experience each day while
                      keeping all shopping & checkout flows 100% identical.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {(form.daily_atmosphere_enabled ?? true) ? (
                      ...
                      <Badge
                        variant="outline"
                        className="bg-muted/50 text-muted-foreground border-border text-[10px] font-medium px-2 py-0.5"
                      >
                        ○ Disabled (Static)
                      </Badge>
                    )}
                    <Switch
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        toast.success(
                          checked
                            ? "Infinite Daily Atmosphere enabled!"
                            : "Daily Atmosphere disabled. Using static theme color.",
                        );
                      }}
                    />
                  </div>
                ...
                {(() => {
                  ...
                  return (
                    <div
                      className={`rounded-xl border p-3 text-xs transition-all ${isEnabled ? "border-primary/30 bg-card shadow-2xs" : "border-border/60 bg-muted/40 opacity-70"}`}
                    >
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <Label className="text-sm font-semibold">
                      App Brand & Theme Color (Fallback / Static Mode)
                    </Label>
                    <p className="text-[11px] text-muted-foreground">
                      Sets the primary accent color when Daily Atmosphere is disabled.
                    </p>
                  </div>
                  <Button
                    type="button"
                ...
                <div className="flex flex-wrap items-center gap-3 pt-1">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    {[
                      ...
                      const isSelected =
                        (form.theme_color || "#0ea5e9").toLowerCase() ===
      ...
      },
      {
        ...
        description:
          "Create and manage fulfillment hubs, store codes, tax GSTINs, delivery zones, and manager assignments.",
        keywords: [
          "branch",
          "branches",
          "hub",
      ...
      },
      {
        ...
        description:
          "Exact store GPS coordinates used for route calculations, distance delivery fees, and driver navigation.",
        keywords: [
          "location",
          "gps",
          "pin",
          ...
          <Card className="border-border/70 shadow-xs">
            <CardHeader className="pb-3 flex flex-row items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Exact store coordinates used for route calculations, distance delivery fees, and
                  driver navigation.
                </p>
              </div>
              <Button
            ...
            <CardContent className="space-y-3">
              {form.shop_lat && form.shop_lng ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <code className="text-primary font-mono">
                        {Number(form.shop_lat).toFixed(6)}, {Number(form.shop_lng).toFixed(6)}
                      </code>
                    </div>
                  </div>
                  <Button
                ...
                <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-2.5 text-xs text-amber-700 dark:text-amber-300">
                  ⚠️ Store GPS pin not set. Click &quot;Move Pin on Map&quot; to drop a doorstep pin
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div>
                  ...
                  <Input
                    ...
                    value={form.shop_lat ?? ""}
                    onChange={(e) =>
                      setForm({ ...form, shop_lat: e.target.value ? Number(e.target.value) : null })
                    }
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={form.shop_lng ?? ""}
                    onChange={(e) =>
                      setForm({ ...form, shop_lng: e.target.value ? Number(e.target.value) : null })
                    }
                    className="rounded-xl text-sm"
                  />
                </div>
      ...
      },
      {
        ...
        description:
          "Accept orders locally, state-wide across all over Tamil Nadu, Pan-India, or Worldwide Export with custom courier rates and SLA.",
        keywords: [
          "shipping",
          "scope",
          "tamil nadu",
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Configure whether this store accepts orders locally within city radius,
                    state-wide across all of Tamil Nadu, Pan-India, or Worldwide.
                  </p>
                </div>
                <Badge
                ...
                >
                  ...
                  {shippingScope.scope_mode === "state" &&
                    `🗺️ State-Wide (${shippingScope.state_name})`}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  >
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed">
                      Hyperlocal doorstep delivery within store GPS radius (
                      {shippingScope.local_radius_km} km) via bike riders.
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed">
                      Accepts orders across all districts of{" "}
                      {shippingScope.state_name || "Tamil Nadu"} via inter-district cold-chain
                      parcel courier.
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed">
                      Accepts orders from all 28 states &amp; 8 UTs via domestic express air cargo
                      &amp; speed logistics.
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed">
                      Cross-border global export shipping via DHL / FedEx International with customs
                      compliance.
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed">
                      Delivers locally within {shippingScope.local_radius_km} km radius, regional
                      cold-chain within {shippingScope.state_name}, and courier for other states /
                      global orders.
                    </p>
                  </button>
                </div>
              ...
              {(shippingScope.scope_mode === "state" || shippingScope.scope_mode === "hybrid") && (
                <div className="rounded-2xl border border-indigo-500/30 bg-indigo-500/5 p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[10px] border-indigo-500/40 text-indigo-700 dark:text-indigo-300"
                    >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({ ...prev, state_name: e.target.value }))
                        }
                        placeholder="e.g. Tamil Nadu"
                        className="mt-1 rounded-xl text-xs"
                      />
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        Default state for regional orders
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            state_shipping_fee: Number(e.target.value),
                          }))
                        }
                        className="mt-1 rounded-xl text-xs"
                      />
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        Flat courier charge per shipment
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            state_free_over: Number(e.target.value),
                          }))
                        }
                        className="mt-1 rounded-xl text-xs"
                      />
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        Waive courier fee above this total
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({ ...prev, state_sla_text: e.target.value }))
                        }
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            state_courier_partners: e.target.value,
                          }))
                        }
              ...
              {(shippingScope.scope_mode === "national" ||
                ...
                <div className="rounded-2xl border border-purple-500/30 bg-purple-500/5 p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[10px] border-purple-500/40 text-purple-700 dark:text-purple-300"
                    >
                      All 28 States &amp; 8 UTs
                    </Badge>
                  </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            national_shipping_fee: Number(e.target.value),
                          }))
                        }
                        className="mt-1 rounded-xl text-xs"
                      />
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            national_free_over: Number(e.target.value),
                          }))
                        }
                        className="mt-1 rounded-xl text-xs"
                      />
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            national_sla_text: e.target.value,
                          }))
                        }
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            national_courier_partners: e.target.value,
                          }))
                        }
              ...
              {(shippingScope.scope_mode === "international" ||
                ...
                <div className="rounded-2xl border border-cyan-500/30 bg-cyan-500/5 p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[10px] border-cyan-500/40 text-cyan-700 dark:text-cyan-300"
                    >
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            international_shipping_fee: Number(e.target.value),
                          }))
                        }
                        className="mt-1 rounded-xl text-xs"
                      />
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            international_min_order: Number(e.target.value),
                          }))
                        }
                        className="mt-1 rounded-xl text-xs"
                      />
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            international_sla_text: e.target.value,
                          }))
                        }
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setShippingScope((prev) => ({
                            ...prev,
                            international_customs_note: e.target.value,
                          }))
                        }
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setShippingScope((prev) => ({
                      ...prev,
                      custom_packaging_notice: e.target.value,
                    }))
                  }
                ...
                />
                <p className="text-[11px] text-muted-foreground mt-1">
                  Displayed in the order summary at checkout when outstation / courier shipping is
                  triggered.
                </p>
              </div>
      ...
      },
      {
        ...
        description:
          "Base delivery fee, free delivery over threshold, per-km distance pricing, and maximum delivery radius.",
        keywords: [
          "delivery",
          "fee",
          "pricing",
      ...
      },
      {
        ...
        description:
          "Enable express 30/45 minute priority delivery dispatch with custom surcharge fee.",
        keywords: [
          "express",
          "sla",
          "turnaround",
          ...
          <Card className="mt-6 rounded-2xl shadow-xs">
            <CardHeader>
              <CardTitle className="text-base flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <Zap className="size-4 text-amber-500 fill-amber-500" />⚡ Express Delivery
                  Turnaround & SLA Settings
                </span>
                <Badge
                  variant="outline"
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between rounded-xl border border-border bg-muted/20 p-3">
                <div className="space-y-0.5">
                  <Label className="text-sm font-semibold">
                    Enable Express Turnaround (30–45 Mins)
                  </Label>
                  <p className="text-xs text-muted-foreground">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, express_delivery_fee: Number(e.target.value) })
                    }
                    className="mt-1"
                  />
                  <p className="text-[11px] text-muted-foreground mt-1">
      ...
      },
      {
        ...
        description:
          "Enable or disable customer ability to request returns. Requires product-level return window setup.",
          ...
          <Card className="mb-6 border-border/80 shadow-xs">
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">
                When enabled, customers can request returns for items that have a defined{" "}
                <strong>Return Window (Days)</strong> set in the Catalog. Refunds will need to be
                processed manually by staff via the Orders panel.
              </p>
            </CardContent>
          </Card>
        ),
      },
      {
        ...
        description:
          "Real-time announcement ticker on customer app showing fresh harbour arrivals and boat timings.",
        keywords: [
          "catch",
          "alerts",
          "boat",
            ...
            <CardHeader className="pb-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Control the top announcement alert banner shown to customers. Turn OFF if your
                    store is inland or without boat landings.
                  </p>
                </div>
                <div className="flex items-center gap-2">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  <p className="text-[11px] text-muted-foreground mt-1">
                    Appears on the pill badge in the top banner (e.g. Kasimedu Harbour, Cochin
                    Harbour, Bio-Secure Farm Hub).
                  </p>
                </div>
                <div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-border/60">
                <p className="text-[11px] text-muted-foreground">
                  Toggle at top right takes effect immediately. Click below to save custom title &
                  message.
                </p>
                <Button
                  type="button"
      ...
      },
      {
        ...
        description:
          "Cash on Delivery (COD) switch, Razorpay / Stripe credentials, and online payment methods.",
        keywords: [
          "payment",
          "payments",
          "cod",
            ...
            <CardContent className="space-y-4">
              ...
              <div className="rounded-xl border p-4 bg-muted/40 space-y-3">
                <h4 className="font-semibold text-sm">
                  Direct UPI & QR Code Settings (Zero Gateway Fees)
                </h4>
                <p className="text-xs text-muted-foreground">
                  Payments go directly to your bank account via UPI with 0% commission. Customers
                  can scan a QR code or tap to pay using GPay, PhonePe, or Paytm.
                </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                      ...
                      <Input
                        ...
                        value={
                          gatewayForm.secret_key
                            ? "••••••••" + gatewayForm.secret_key.slice(-4)
                            : ""
                        }
                        onChange={(e) =>
                          setGatewayForm({ ...gatewayForm, secret_key: e.target.value })
                        }
                      ...
                      <Input
                        ...
                        value={
                          gatewayForm.webhook_secret
                            ? "••••••••" + gatewayForm.webhook_secret.slice(-4)
                            : ""
                        }
                        onChange={(e) =>
                          setGatewayForm({ ...gatewayForm, webhook_secret: e.target.value })
                        }
                ...
                {gatewayForm.provider !== "none" && gatewayForm.provider !== "razorpay" && (
                  <>
                    <div>
                      <Label>
                        {gatewayForm.provider === "stripe"
      ...
      },
      {
        ...
        description:
          "Default checkout tender selection and custom payment modes (Sodexo, Khata, Swiggy POS, Cheque) isolated per tenant store.",
        keywords: [
          "pos payment",
          "tender",
          "sodexo",
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Tenant-isolated payment modes. Default tender is pre-selected on the counter
                    billing drawer.
                  </p>
                </div>
            ...
            <CardContent className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <Label className="text-xs font-bold text-foreground">
                      Default POS Counter Payment Tender
                    </Label>
                    <p className="text-[11px] text-muted-foreground">
                      Pre-selected payment method when opening the counter bill drawer for fast
                      1-click checkout.
                    </p>
                  </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Define client-specific tenders like Sodexo/Pluxee, Khata/Store Credit, Swiggy
                      POS, Cheques, etc.
                    </p>
                  </div>
                ...
                <div className="space-y-2">
                  {paymentConfig.customMethods.length === 0 ? (
                    <div className="p-6 text-center text-xs text-muted-foreground border border-dashed rounded-2xl">
                      No custom payment methods defined. Add one below to offer custom billing
                      tenders at the counter.
                    </div>
                  ) : (
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] font-mono text-muted-foreground">
                            ID: <code className="text-foreground">{cm.id}</code> • Placeholder: "
                            {cm.refPlaceholder || "Ref #"}"
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
                          <div className="flex items-center gap-2">
                            ...
                            <Switch
                              ...
                              onCheckedChange={(checked) =>
                                handleToggleCustomMethod(cm.id, checked)
                              }
                            />
                          </div>
                          <Button
              ...
              <div className="p-4 rounded-2xl border border-primary/20 bg-primary/5 space-y-3.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs font-semibold">
                      Reference / Slip Field Placeholder
                    </Label>
                    <Input
      ...
      },
      {
        ...
        description:
          "Statutory details printed automatically on GST Tax Invoices and thermal receipts.",
        keywords: [
          "gst",
          "gstin",
          "tax",
        ...
        content: (
          <Card className="md:col-span-2">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" asChild className="rounded-xl text-xs h-7">
                    <Link to="/licence" target="_blank">
                      <FileText className="size-3 mr-1" /> View SLA &amp; License{" "}
            ...
            <CardContent className="space-y-4">
              {/* Master GST Calculation Switch */}
              <div className="flex items-center justify-between p-4 rounded-2xl border bg-muted/20">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Label htmlFor="master_gst_toggle" className="text-sm font-bold cursor-pointer">
                      Master GST Tax Calculation &amp; Invoicing
                    </Label>
                    {form.gst_enabled !== false ? (
                  ...
                  <p className="text-xs text-muted-foreground max-w-xl">
                    When turned <strong className="text-foreground">OFF</strong>, 0% GST is
                    calculated across all sales channels: POS counter billing, online carts,
                    checkout, customer receipts, and database orders. Disable this if your business
                    operates below the GST threshold or under composition scheme.
                  </p>
                </div>
                <Switch
                  id="master_gst_toggle"
                  checked={form.gst_enabled !== false}
              ...
              <div>
                <Label htmlFor="terms_and_conditions">
                  Terms and Conditions &amp; Return Policies
                </Label>
                <textarea
      ...
      },
      {
        ...
        description:
          "Default GST tax percentage, low stock alert threshold, and social buying urgency badges.",
        keywords: [
          "gst rate",
          "tax",
          "percent",
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, default_gst_percent: Number(e.target.value) })
                    }
                    placeholder="0"
                    className="mt-1"
                  />
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, stock_urgency_threshold: Number(e.target.value) })
                    }
                    placeholder="5"
                    className="mt-1"
                  />
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    Displays "🔥 Only X left!" to customers when stock is below or equal to this
                    limit.
                  </p>
                </div>
                ...
                <div className="flex flex-col justify-between rounded-xl border p-3 bg-muted/20">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Displays remaining quantity badge to customers in catalog & product page to
                      drive conversions.
                    </p>
                  </div>
                  <div className="pt-2 flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, show_stock_to_customers: e.target.checked })
                      }
      ...
      },
      {
        ...
        tab: "payments" as const,
        tabLabel: "Payments & Tax",
        title: "Thermal Billing Printer & Slip Customizer",
        description:
          "Customize receipt headers, GSTIN, FSSAI, WhatsApp support, and social handles with live preview.",
        keywords: [
          "printer",
          "thermal",
          "receipt",
        ...
        content: (
          <div className="mt-4">
            <ThermalPrinterCustomizer
              ...
              onSaveToDatabase={async (patch) => {
                if (settings?.id) {
                  await supabase.from("store_settings").update(patch).eq("id", settings.id);
                  qc.invalidateQueries({ queryKey: ["store_settings"] });
                }
              }}
            />
      ...
      },
      {
        ...
        description:
          "Dynamic canonical domain, Schema.org rich snippets, meta titles, social share preview cards, and Google/Bing tokens.",
        keywords: [
          "seo",
          "meta",
          "google",
      ...
      },
      {
        ...
        description:
          "Customer Refer & Earn program toggle, store credit cashback %, and maximum checkout wallet burn percent.",
        keywords: [
          "wallet",
          "referral",
          "refer and earn",
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between rounded-xl border border-border bg-muted/20 p-3">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    When enabled, customers get cashback credited on delivered orders, and can
                    redeem balance at checkout.
                  </p>
                </div>
                <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center justify-between rounded-xl border border-border bg-muted/20 p-3">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    When turned OFF, all referral modals, invite chips, account referral cards, and
                    checkout invite code inputs are completely hidden from customers.
                  </p>
                </div>
                <div className="flex items-center gap-2.5">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(val) =>
                      setForm({ ...form, referral_program_enabled: val } as any)
                    }
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, referral_reward_referrer: Number(e.target.value) })
                    }
                    className="mt-1"
                  />
                  <p className="text-[11px] text-muted-foreground mt-1">
                    FreshCash credited to the user who shared the code upon friend's first delivered
                    order.
                  </p>
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, referral_reward_referee: Number(e.target.value) })
                    }
                    className="mt-1"
                  />
                  <p className="text-[11px] text-muted-foreground mt-1">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, max_wallet_burn_percent: Number(e.target.value) })
                    }
                    className="mt-1"
                  />
                  <p className="text-[11px] text-muted-foreground mt-1">
                    Max percentage of order subtotal that can be paid using FreshCash balance (e.g.
                    50%).
                  </p>
                </div>
              </div>
      ...
      },
      {
        ...
        description:
          "Push notification credentials for instant order status alerts on mobile browsers and Android PWAs.",
        keywords: [
          "fcm",
          "push",
          "firebase",
            ...
            <CardContent className="space-y-4">
              <div className="rounded-xl border border-sky-500/20 bg-sky-50/50 dark:bg-sky-950/20 p-3 text-xs text-sky-800 dark:text-sky-300">
                ...
                <p className="mt-1 opacity-90">
                  Customer & staff device tokens are automatically captured in PostgreSQL{" "}
                  <code className="font-mono bg-sky-100 dark:bg-sky-900/60 px-1 py-0.5 rounded">
                    fcm_tokens
                  </code>{" "}
                  table. The system operates locally with service worker push fallback. When you are
                  ready to configure your Firebase project, enter your credentials below.
                </p>
              </div>
      ...
      },
      {
        ...
        description:
          "Automatic PDF order confirmation and invoice email dispatch to customers upon placing orders.",
        keywords: [
          "email",
          "emails",
          "smtp",
          ...
          <Card className="mt-6 rounded-2xl shadow-xs border-sky-500/20">
            <CardHeader>
              <CardTitle className="text-base flex items-center justify-between">
                ...
                <Badge
                  variant="outline"
                  className="text-xs border-sky-500/40 text-sky-600 dark:text-sky-400"
                >
                  Resend & SMTP Ready
                </Badge>
              </CardTitle>
            ...
            <CardContent className="space-y-4 text-xs">
              <p className="text-muted-foreground">
                Configure the verified sender domain and API key. When orders are placed, confirmed,
                or delivered, branded HTML receipts with attached GSTIN Tax Invoices will be
                dispatched to customers automatically.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    ...
                    . If blank, falls back to simulated/env mode.
                  </p>
                </div>
      ...
      },
      {
        ...
        description:
          "Toggle optional commercial modules like Culinary AI, Driver Route Optimization, Live Chat, and Retail POS.",
        keywords: [
          "modules",
          "features",
          "ai benefits",
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex items-center justify-between rounded-xl border border-border/70 p-3.5 bg-muted/20">
                  <div className="space-y-0.5 pr-2">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      In-store barcode scanning, quick cash/QR billing, and direct ESC/POS thermal
                      printing.
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setForm({ ...form, feature_pos_enabled: checked })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between rounded-xl border border-border/70 p-3.5 bg-muted/20">
                  <div className="space-y-0.5 pr-2">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Real-time two-way messaging desk for customer inquiries, order assistance, and
                      WhatsApp dial.
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setForm({ ...form, feature_live_chat_enabled: checked })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between rounded-xl border border-border/70 p-3.5 bg-muted/20">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setForm({ ...form, feature_wallet_enabled: checked })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between rounded-xl border border-border/70 p-3.5 bg-muted/20">
                  <div className="space-y-0.5 pr-2">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Dynamic health benefits, cooking tips, and nutrient breakdowns on product
                      pages.
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setForm({ ...form, feature_ai_benefits_enabled: checked })
                      }
                    />
                  </div>
                </div>
      ...
      },
      {
        ...
        description:
          "Support hotline phone, customer service email, and allowable post-delivery complaint window in hours.",
        keywords: [
          "customer service",
          "support",
          "phone",
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, complaint_window_hours: Number(e.target.value) })
                  }
                />
                <p className="mt-1 text-[10px] text-muted-foreground">
      ...
      },
      {
        ...
        description:
          "1-click JSON database backups, Orders CSV ledger export, Products catalog CSV export, and schema audit.",
        keywords: [
          "data",
          "export",
          "backup",
            ...
            <CardHeader>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Complete database portability, 1-click JSON backup, CSV ledger exports, and
                    schema verification.
                  </p>
                </div>
            ...
            <CardContent className="space-y-4">
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-50/50 dark:bg-emerald-950/20 p-3.5 text-xs text-emerald-900 dark:text-emerald-200">
                ...
                <p className="mt-1 opacity-90 leading-relaxed">
                  All product catalogs, order history, inventory logs, and customer records belong
                  unconditionally to your business. Download regular backups to safeguard against
                  accidental changes or migrate to any Postgres environment.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="rounded-2xl border border-border/80 bg-card p-4 space-y-2.5 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">
                      Exports products, orders with items, categories, marketing campaigns, and
                      store settings.
                    </p>
                  </div>
                  <Button
                ...
                <div className="rounded-2xl border border-border/80 bg-card p-4 space-y-2.5 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">
                      Spreadsheet export of sales, payment methods, delivery fees, and customer
                      contact data.
                    </p>
                  </div>
                  <Button
                ...
                <div className="rounded-2xl border border-border/80 bg-card p-4 space-y-2.5 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">
                      Export product SKU lists with retail prices, inward cost prices, HSN
                      classification, and stock units.
                    </p>
                  </div>
                  <Button
                    type="button"
  ...
  const TAB_DEFINITIONS = [
    {
      ...
      label: "General & Brand",
      icon: Store,
      count: SETTINGS_SECTIONS.filter((s) => s.tab === "general").length,
    },
    {
      id: "branches" as const,
      label: "Branches & Hubs",
      icon: MapPin,
      count: SETTINGS_SECTIONS.filter((s) => s.tab === "branches").length,
    },
    {
      id: "delivery" as const,
      label: "Delivery & Logistics",
      icon: Truck,
      count: SETTINGS_SECTIONS.filter((s) => s.tab === "delivery").length,
    },
    {
      id: "payments" as const,
      label: "Payments & Tax",
      icon: CreditCard,
      count: SETTINGS_SECTIONS.filter((s) => s.tab === "payments").length,
    },
    {
      id: "growth" as const,
      label: "Marketing & Growth",
      icon: Sparkles,
      count: SETTINGS_SECTIONS.filter((s) => s.tab === "growth").length,
    },
    {
      id: "system" as const,
      label: "System & Modules",
      icon: SlidersHorizontal,
      count: SETTINGS_SECTIONS.filter((s) => s.tab === "system").length,
    },
  ];
      ...
      {searchQuery.trim() ? (
        <div className="space-y-6">
          <div className="flex items-center justify-between gap-2 p-3 rounded-2xl bg-primary/5 border border-primary/20">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span>
                Found <strong>{filteredSections.length}</strong> matching setting section
                {filteredSections.length === 1 ? "" : "s"} for "<strong>{searchQuery}</strong>"
              </span>
            </div>
            <Button
              variant="ghost"
          ...
          {filteredSections.length > 0 ? (
              ...
              {filteredSections.map((section: SettingSectionItem) => (
                <div key={section.id} className="space-y-2">
                  <div className="flex items-center gap-2 px-1">
                    <Badge
                      variant="outline"
                      className="text-[10px] font-bold border-primary/30 text-primary bg-primary/5 uppercase tracking-wider"
                    >
                      {section.tabLabel}
                    </Badge>
            ...
            <div className="rounded-3xl border border-dashed border-border/80 p-12 text-center bg-card/50">
              ...
              <p className="text-xs text-muted-foreground mt-1 max-w-sm mx-auto">
                Try searching for a different keyword like "GST", "Logo", "Branch", "Delivery",
                "Wallet", or "SEO".
              </p>
        ...
        >
          ...
          <TabsContent value="general" className="space-y-6 mt-0">
            {SETTINGS_SECTIONS.filter((s: SettingSectionItem) => s.tab === "general").map(
              (s: SettingSectionItem) => (
          ...
          <TabsContent value="branches" className="space-y-6 mt-0">
            {SETTINGS_SECTIONS.filter((s: SettingSectionItem) => s.tab === "branches").map(
              (s: SettingSectionItem) => (
                <div key={s.id}>{s.content}</div>
              ),
            )}
          ...
          <TabsContent value="delivery" className="space-y-6 mt-0">
            {SETTINGS_SECTIONS.filter((s: SettingSectionItem) => s.tab === "delivery").map(
              (s: SettingSectionItem) => (
                <div key={s.id}>{s.content}</div>
              ),
            )}
          ...
          <TabsContent value="payments" className="space-y-6 mt-0">
            {SETTINGS_SECTIONS.filter((s: SettingSectionItem) => s.tab === "payments").map(
              (s: SettingSectionItem) => (
                <div key={s.id}>{s.content}</div>
              ),
            )}
          ...
          <TabsContent value="growth" className="space-y-6 mt-0">
            {SETTINGS_SECTIONS.filter((s: SettingSectionItem) => s.tab === "growth").map(
              (s: SettingSectionItem) => (
                <div key={s.id}>{s.content}</div>
              ),
            )}
          ...
          <TabsContent value="system" className="space-y-6 mt-0">
            {SETTINGS_SECTIONS.filter((s: SettingSectionItem) => s.tab === "system").map(
              (s: SettingSectionItem) => (
                <div key={s.id}>{s.content}</div>
              ),
            )}
        ...
        <div className="flex items-center gap-2 min-w-0">
          <span
            className={
              ...
              ((form.is_open ?? true) ? "bg-emerald-500 animate-pulse" : "bg-destructive")
            }
          />
